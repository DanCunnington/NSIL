...
:- module(metagol,[metaabd/1, metaabd/2, learn/2,learn/5,learn_seq/2,pprint/1,new_var/1,get_labels/1,reset_newvars/0,op(950,fx,'@')]).
...
metaabd(Pos):-
    ...
    set_labels(Pos),
    reset_newvars,
    learn(Pos,[],Prog,Abd,_S), %% abduce constraints
    best_labels(Abd, Score1, Best_Label), %% Abduce best labels
    ...
    set_best_program(Prog,Best_Label),
    %% if no need for abduction, then terminate
    (   Score < 1.0 -> false; metaabd_terminate).
metaabd(_):-
    metaabd_terminate.
metaabd(Pos,Neg):-
    set_best_prob(-1),
    ...
    set_labels(Pos),
    reset_newvars,
    learn(Pos,Neg,Prog,Abd,_S), %% abduce constraints
    best_labels(Abd, Score1, Best_Label), %% Abduce best labels
    prog_prob(Prog,Score2),
    Score is Score1 * Score2,
    is_better(Score),
...
metaabd_terminate :-
    ...
    (   Abd \= [] -> writeln(Abd); writeln('None')).

set_labels(Exs) :-
    all_labels(Exs, Labels),
    nb_setval(labels, Labels),
    length(Labels, N),
    nb_setval(n_vars, N).
get_labels(Labels) :-
    nb_getval(labels, Labels).

all_labels([], []).
all_labels([Ex|Exs], Labels) :-
    Ex =.. [_P, Labels1, _Out],
    all_labels(Exs, Labels2),
    append(Labels1, Labels2, Labels).

reset_newvars :-
    nb_setval(c_vars, 0).

final_score(Prog,Score,Score1) :-
    prog_size(Prog, K),
    Score1 is 6*Score/(pi^2*K^2).
...
prog_size([], 0).
prog_size([sub(_,_,A,_)|Prog], Size) :-
    prog_size(Prog, Size1),
    Size is A + Size1.
