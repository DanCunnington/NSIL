...
from examples.hitting_sets.HittingSetsData import Images
...
def load_nesy_data(root_dir, task_type, image_type):
    train_csv = pd.read_csv(join(root_dir, f'{task_type}', f'{image_type}_train.csv')).values
    test_csv = pd.read_csv(join(root_dir, f'{task_type}', f'{image_type}_test.csv')).values
    return train_csv, test_csv


def load_nn_data(args, root_dir, batch_size):
    train_csv_file = pd.read_csv(f'{root_dir}/{args.task_type}/{args.image_type}_train.csv')
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.1307,), (0.3081,))
    ])

    im_dir = join(IMAGE_DATA_DIR, f'{args.image_type}_1_to_5')
    image_data = {
        'train': Images(root_dir=join(im_dir, 'train'), transform=transform, gray=True),
        'test': Images(root_dir=join(im_dir, 'test'), transform=transform, gray=True)
    }
