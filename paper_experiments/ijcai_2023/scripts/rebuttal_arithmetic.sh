...
do
  ...
  then
    ...
    mkdir -p $RESULTS_DIR
    python run_with_given_h.py --pct ${DATA_PCT} --save_dir $RESULTS_DIR --seed ${seeds[SEED_IDX]} --skip_symbolic_learning --skip_corrective_examples --save_nets ${ADDITIONAL_ARGS}
    COUNT=$((COUNT+1))
  fi
done