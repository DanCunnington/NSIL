...
if [[ "$ADDITIONAL_ARGS" == *"task_type=prod"* ]]; then
  SUB_RESULT_DIR="meta_abd_data_prod"
else
  SUB_RESULT_DIR="meta_abd_data_sum"
fi

SCRIPT_DIR=$( cd -- "$( dirname -- "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )

EXPERIMENT=recursive_arithmetic
...
do
  ...
  then
    ...
    RESULTS_DIR="results/repeats/${SUB_RESULT_DIR}/${REPEAT_ID}"

    mkdir -p $RESULTS_DIR
    python run.py --ilp_system=ILASP --custom_ilp_cmd_line_args='--version=4' --meta_abd_data --num_iterations=50 --custom_eval_interval=50 --prune_ilp_example_weight_threshold=5 --pct ${DATA_PCT} --save_dir $RESULTS_DIR --seed ${seeds[SEED_IDX]} --save_nets ${ADDITIONAL_ARGS}
    COUNT=$((COUNT+1))
  fi
done